...
void main() {
  ...
  group('divide с числом знаков вне досягаемости степени десятки', () {
    // Масштаб больше миллиона получается обычным умножением, и тогда
    // показатель, в который сворачиваются оба масштаба и число знаков,
    // выходит за границу степени десятки. Прямой путь такое не считает и
    // уступает дорогу прежнему — через выравненную пару.
    final huge = ShortDecimal(1, shiftRight: 750000) *
        ShortDecimal(1, shiftRight: 750000);

    test('масштаб полтора миллиона делится и округляется в ноль', () {
      expect(huge.scale, 1500000);
      expect(
        huge.divide(ShortDecimal(3), scaleOnInfinitePrecision: 10),
        ShortDecimal.zero,
      );
    });

    test('и с делителем, для которого дроби не существует', () {
      // Знаменателю 2^63 негде быть положительным в int64, поэтому пара
      // округляется без построения дроби.
      const min = -9223372036854775808;

      expect(
        huge.divide(ShortDecimal(min), scaleOnInfinitePrecision: 10),
        ShortDecimal.zero,
      );
      expect(
        huge.divide(ShortDecimal(min), scaleOnInfinitePrecision: 0),
        ShortDecimal.zero,
      );
    });
  });
