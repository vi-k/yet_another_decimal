...
void main() {
  group('ShortDecimal', () {
    ...
    // Д26: сумма, у которой каноническая форма помещается, считается точно;
    // сумма, у которой не помещается, переполняется молча — как и обещано.
    group('сложение на границе int64', () {
      test('канонический результат помещается — значит считается', () {
        const max = 9223372036854775807;

        expectShortDecimal(
          ShortDecimal(max) + ShortDecimal(3),
          '9223372036854775810',
        );
        expectShortDecimal(
          ShortDecimal(max) - ShortDecimal(-3),
      ...
      test('не помещается — переполнение остаётся молчаливым', () {
        const max = 9223372036854775807;

        // 9223372036854775808 нулём не заканчивается: в масштаб переносить
        // нечего, и ответ заворачивается, как обещает семейство.
        expect(
          (ShortDecimal(max) + ShortDecimal.one).toString(),
          '-9223372036854775808',
        );
      });
    });
