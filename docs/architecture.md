...
семейства классов. Дублирование **реализации** здесь сознательное и
подтверждено замером: общая реализация над абстракцией стоит вдвое дороже
из-за боксинга `int` в поле типа `T`.

> **Уточнено 2026-08-25.** Долгое время здесь было записано, что дорог сам
> общий интерфейс. Это неверно: измерение показало **0 %** — при конкретном
> статическом типе получателя вызов у класса, реализующего интерфейс, стоит
> столько же, сколько без него. Доказательство от противного есть в самом
> пакете: оба класса реализуют `Comparable<T>` и остаются быстрыми. Дорога
> только общая **реализация**, а не общий **контракт**. Поэтому в 1.2.0
> добавляется внутренний интерфейс `FixedPoint`, который ловит расхождения
> семейств ошибкой компиляции. Подробности — в
> `docs/records/2026-08-25[4]-package-rework-design.md`.
...
Обоснование верно **наполовину**, и это важно знать до правок. Замер внутри
пакета (2026-08-25, Dart 3.13):

- на паре «большое делимое, маленький делитель» — а именно такую и порождает
  деление — Евклид схлопывает 81-значное число в 9-значное одним взятием
  остатка и **выигрывает у штатного `BigInt.gcd` в 2.7 раза**;
- на паре сопоставимой величины (например, в `Fraction` из длинных чисел)
  штатный двоичный алгоритм, наоборот, **выигрывает в 2.5 раза**.

Поэтому целевая реализация выбирает алгоритм по величине аргументов, а не
заменяет один другим. Цифры и код — в
`docs/records/2026-08-25[3]-package-review.md`, раздел П1.
